...
## 在您的机器上运行 Sōzu，使用虚拟后端

### 创建一个包含 HTTP 和 HTTPS 监听器的配置

`bin/config.toml` 的大多数默认值都是合理的，将它们复制到 `bin/my-config.toml`。
一个需要调整的重要值是 `worker_count`。仅测试一个
工作进程的性能似乎是理想的：
...
它们位于 `/lib/assets/`。
但这样您必须使用 `lolcatho.st` 作为本地域名，
并确保您的 `/etc/hosts` 中有这些行：
...
创建四个简单的 HTTP 服务器，当通过
HTTP GET 请求 `/api` 路径时，每个服务器都回复 200 OK（随意选择）。
使用您喜欢的编程语言，这应该很简单。
这是 [8 行 javascript 代码](https://www.digitalocean.com/community/tutorials/how-to-create-a-web-server-in-node-js-with-the-http-module)。
确保每个后端都在 localhost 上监听，并且端口不同，
例如：1051、1052、1053、1054。

将这些后端添加到 `my-config.toml` 的 frontends 下：
...
提示：重命名 release 二进制文件并将其放入 cargo 路径：

```bash
mv target/release/sozu $HOME/.cargo/bin/sozu-0.15.14
```

现在您可以以 release 模式运行 Sōzu：
...
[`tls-perf`](https://github.com/tempesta-tech/tls-perf) 是一个用 C 编写的 TLS 压力测试工具。
它没有依赖项，并且易于构建。
...
进程（Sōzu）和 linux 内核之间的交互。在大多数 linux 发行版中它都是一个可用的包。
...
使用 strace 跟踪此 pid。您可能需要 root 权限。

```bash
strace --attach=14157
```

您应该会看到常规的 `epoll_wait` 系统调用，这是 Sōzu 的主循环。
...
如果您执行其中一个 curl，您将看到 Sōzu 在
HTTP 或 HTTPS 请求期间执行的所有系统调用。
以下是在简单后端上进行 HTTP GET 期间发生的系统调用。
这只是在监听器套接字上接受流量。
...
现在您可以看到每个系统调用是由哪段代码负责的。
例如，以下是负责在文件描述符 12 的套接字上进行 `connect` 系统调用的代码。
