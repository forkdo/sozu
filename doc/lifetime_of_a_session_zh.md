...
在本文档中，我们将探讨客户端会话从套接字
创建到关闭的整个过程，以及描述
HTTP 请求和响应如何发生的所有步骤。

在深入探讨会话的创建、生命周期和消亡之前，我们需要
了解 Sōzu 中完全分离的两个概念：

- 使用 [mio](https://docs.rs/mio/0.7.13/mio/) crate 处理套接字
- `SessionManager` 跟踪所有会话

### mio 做什么？

Mio 允许我们监听套接字事件。例如，我们可以使用 TCP 套接字
等待连接，当该套接字可读或当套接字有
数据要读取，或被对等方关闭，或计时器触发时，mio 会通知我们...

Mio 提供了对 linux 系统调用 [epoll](https://man7.org/linux/man-pages/man7/epoll.7.html) 的抽象。
此 epoll 系统调用允许我们注册文件描述符，以便内核在
这些文件描述符发生任何事情时通知我们。

归根结底，套接字只是原始文件描述符。我们使用 mio
`TcpListener`、`TcpStream` 包装器来包装这些文件描述符。`TcpListener`
在特定端口上侦听连接。对于每个新连接，它都会创建一个
`TcpStream`，后续流量将重定向到该 `TcpStream`（来自客户端和到客户端）。
...
工作是将 Token 链接到一个将使用该订阅的 `ProxySession`。
这是使用 [Slab]() 完成的，Slab 是一个优化内存使用的键值数据结构：
...
代理管理侦听器、前端、后端以及与
每个协议相关的业务逻辑（缓冲、解析、错误处理...）。

## 接受连接

Sōzu 使用 TCP 侦听器套接字来获取新连接。它通常会侦听
端口 80（HTTP）和 443（HTTPS），但也可能有其他用于 TCP 代理的端口，
...
Sōzu 可热重新配置。我们可以在运行时添加侦听器和前端。对于每个添加的侦听器，
...
每当客户端连接到前端时：
1. 它到达一个侦听器
2. Sōzu 收到 mio 的通知，即在特定 Token 上收到了一个 `readable` 事件
3. 使用 SessionManager，Sōzu 获取相应的 `ListenSession`
4. Sōzu 确定使用的协议
5. Token 被传递给适当的代理（TCP/HTTP/HTTPS）
6. 使用 Token，代理确定哪个 `TcpListener` 触发了事件
7. 代理开始在一个循环中从中接受新连接（因为可能不止一个）
...
当我们接受一个新连接（`TcpStream`）时，它可能已经在
侦听器队列中等待了一段时间。内核甚至可能已经有一些可用的数据，
例如 HTTP 请求。如果我们处理该请求太慢，客户端可能
在我们将会话转发到
后端并且后端响应时已经断开连接（超时）。

如果达到最大客户端连接数（由配置中的 `max_connections` 提供），
新的连接将保留在队列中。
如果队列已满，我们将丢弃新接受的连接。
通过指定最大并发连接数，我们确保
服务器不会过载并为现有
...
`Session` 结构保存与
[会话相关的数据](https://github.com/sozu-proxy/sozu/blob/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src/https_openssl.rs#L65-L83)：
...
代理为接受队列的每个项目创建一个会话，
使用项目中提供的 `TcpStream`。
`TcpStream` 在 mio 中注册一个新令牌（称为 frontend_token）。
会话以相同的令牌添加到 SessionManager 中。

### 检查僵尸会话

因为在创建和删除会话时可能会发生错误，并且其中一些
可能会被“遗忘”，所以有一个名为
[“僵尸检查器”](https://github.com/sozu-proxy/sozu/blob/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src/server.rs#L446-L496)
的常规任务，它会检查列表中的会话并终止那些卡住或太旧的会话。

## 会话如何从前端套接字读取数据

当数据从网络到达 `TcpStream` 时，它会存储在内核
缓冲区中，并且内核会通知事件循环套接字是可读的。

与侦听套接字一样，与 TCP 套接字关联的令牌将获得一个
“可读”事件，我们将使用该令牌查找它与哪个会话
相关联。然后我们调用
...
让它读取数据、解析等。
该方法将在一个循环中运行 (https://github.com/sozu-proxy/sozu/blob/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src_httpssl.rs#L548-L692),
...
被调用。然后它将调用底层
[状态机](https://github.com/sozu-proxy/sozu/blob/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src_httpssl.rs#L58-L63) 的相同方法。

状态机是实现协议的地方。会话可能需要
在其生命周期内识别不同的协议，具体取决于其配置，
并在它们之间进行[升级](https://github.com/sozu-proxy/sozu/blob/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src/https_openssl.rs#L165)。它们都在[协议目录](https://github.com/sozu-proxy/sozu/tree/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src/protocol)中。

示例：
- HTTPS 会话可以从一个名为 `ExpectProxyProtocol` 的状态开始
- 一旦 expect 协议运行完毕，会话就会升级到 TLS 握手状态：`HandShake`
- 握手完成后，我们有一个 TLS 流，并且会话升级到 `HTTP` 状态
- 如果客户端要求，会话可以切换到 WebSocket 连接：`WebSocket`

现在，假设我们当前正在使用 HTTP 1 协议。会话调用
...
。如果没有错误（套接字关闭等），我们将在
[`readable_parse()`](https://github.com/sozu-proxy/sozu/blob/e4e7488232ad6523791b94ad201239bcf7eb9b30/lib/src/protocol/http/mod.rs#L728) 中处理这些数据。
...
当我们从客户端收到第一个字节时，两者都处于 `Initial` 状态。
...
直到我们达到一个请求状态，其中头部已完全解析。如果没有
足够的数据，我们将等待更多数据到达套接字并重新开始
解析。

一旦我们完成了解析头部，并找到我们正在寻找的内容，我们将
...
会话：
1. 查找要连接到哪个集群
2. 向 SessionManager 请求一个名为 `back_token` 的新有效令牌
3. 请求连接到集群
   - 适当的代理查找后端（添加详细信息）
...
如果 Sōzu 找不到集群，它会向客户端响应一个默认的 HTTP 404 Not Found 响应。
一个会话可以尝试连接到后端 3 次。如果所有尝试都失败，Sōzu 会响应一个默认的
HTTP 503 Service Unavailable 响应。如果 Sōzu 找到了一个集群，但所有相应的
后端都已关闭（或没有响应），则会发生这种情况。

### 保持会话活动

如果 HTTP 请求附带了 Connection 标头设置为 Keep-Alive，
则在收到响应后可以保留底层 TCP 连接，
以发送更多请求。由于 Sōzu 支持在 URL 和
主机名上进行路由，因此下一个请求可能会转到不同的集群。
因此，当我们从请求中获取集群 ID 时，我们会检查它是否与
上一个相同，如果相同，我们会测试后备套接字是否仍然
...
具有相同 id 的 cookie 的请求都将发送到同一个后端服务器。

该查找将根据哪些后端服务器被
认为是有效的（如果它们正确响应）以及为集群配置的负载平衡
策略返回一个结果。
如果找到了后端，我们打开一个到后端服务器的 TCP 连接，
否则我们返回一个 HTTP 503 响应。
...
如上所述，我们有一个名为 `ResponseState` 的小状态机，用于
解析来自后端的流量。整个逻辑基本相同。
...
一旦 `ResponseState` 达到“完成”状态并且每个字节都已发送
回客户端，请求的完整生命周期就结束了。会话
达到 `CloseSession` 状态并从 `SessionManager` 的 slab 中删除
并且其套接字从 mio 中取消注册。
