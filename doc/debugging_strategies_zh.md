...
* `log_target`：日志发送到的位置。它可以有以下格式：
  * `stdout`
  * `udp://127.0.0.1:9876`
  * `tcp://127.0.0.1:9876`
  * `unix:///var/sozu/logs`
  * `file:///var/logs/sozu.log`
* `access_logs_target`：如果激活，则将访问日志发送到单独的目标

`log_level` 遵循 [env_logger 的级别指令](https://docs.rs/env_logger/0.5.13/env_logger/)。
此外，可以使用 `RUST_LOG` 环境变量覆盖日志级别。

如果 sozu 是在发布模式下构建的，则不会编译 `DEBUG` 和 `TRACE` 日志级别，
...
* 通过 `sozu metrics get`，它将显示主进程和工作进程的指标。计数器在每次调用之间刷新
* 通过 UDP，遵循 statsd 协议（可选地支持 InfluxDB 的标签）

以下是如何在配置文件中使用 statsd 设置指标：
...
* 工作进程名称（主进程为“MAIN”）
* 日志级别
* 请求 ID（UUID，为每个请求随机生成，如果在 keep-alive 中执行多个请求，则在同一连接上会更改）
* 集群 ID
* 客户端的源 IP 和端口
* 后端服务器的目标 IP 和端口
* 响应时间（从客户端接收到的第一个字节到发送给客户端的最后一个字节）
...
有全局的 `sozu.bytes_in` 和 `sozu.bytes_out` 指标，用于计算
sozu 的前端流量。
这些指标也可以具有后端 ID 和集群 ID。然后它们将从
后端服务器的角度指示传入和传出的字节。
...
到 HTTPS，到 WSS（基于 TLS 的 websocket）。

您可以跟踪以下仪表，指示当前的协议使用情况：
...
Sozu 有一种以最少的资源使用来响应无效流量的方法，发送预定义的答案。
...
* `sozu.http.404.errors`：未知的主机名和/或路径
* `sozu.http.413.errors`：请求太大
* `sozu.http.503.errors`：无法连接到后端服务器，或相应集群没有可用的后端服务器
...
`sozu.http.503.errors` 指标在请求发回 503 错误后递增，并且在
断路器触发后（我们等待 3 次连接后端服务器失败）发送 503 错误。
...
* `sozu.http.active_requests` 用于当前活动的连接（等待
下一个请求的 keep-alive 连接被标记为不活动）

客户端连接应始终高于后端连接，而后端连接应高于
活动请求（不活动的会话可以保留后端连接）。

这些指标与资源使用密切相关，后者由以下指标跟踪：

* `sozu.slab.entries`：slab 分配器中使用的插槽数。通常，每个监听器套接字有一个插槽，
一个用于连接到主进程，一个用于指标套接字，然后每个前端连接一个，
每个后端连接一个。因此，连接数应始终接近（但低于）slab 计数。
* `sozu.buffer.number`：缓冲池中使用的缓冲区数。不活动的会话和我们发送
默认答案（400、404、413、503 HTTP 错误）的请求不使用缓冲区。活动的 HTTP 会话使用一个缓冲区（
流水线模式除外），WebSocket 会话使用两个缓冲区。因此，缓冲区数应始终低于
slab 计数，并低于连接数。
* `sozu.zombies`：sozu 集成了一个僵尸会话检查器。如果某个会话一段时间内没有任何操作，
则事件循环或协议实现中可能存在错误，因此会记录其内部状态。此计数器
为每个被删除的僵尸会话递增。

新连接被放入队列中，并等待直到会话被创建（如果我们有可用资源），
或直到配置的超时已过。以下指标观察接受队列的使用情况：

* `sozu.accept_queue.connections`：接受队列中的套接字数
* `sozu.accept_queue.timeout`：每次套接字在队列中停留时间过长并被关闭时递增
* `sozu.accept_queue.wait_time`：每次创建会话时，此指标记录套接字在接受队列中必须等待多长时间
...
503 (`sozu.http.503.errors`) 增加，这意味着 sozu 的配置可能有问题。
使用以下命令检查配置状态：
...
它通常伴随着 `sozu.slab.entries` 增加，而连接数或活动请求数
保持不变。然后当僵尸检查器激活时，slab 计数将下降。

### 无效的会话关闭

如果 slab 计数和活动请求保持不变，但 `sozu.client.connections` 和/或 `sozu.backend.connections`
正在增加，这意味着 sozu 没有正确关闭会话，请为此提交问题。
...
。如果 `sozu.accept_queue.timeout` 高于零，sozu 无法足够快地接受会话并且
正在拒绝流量。
...
- 如果错误仅影响其中一个协议（HTTP、HTTPS 或 TCP），请停用其他协议
...
[grad 指标工具](https://github.com/geal/grad) 的开发是为了轻松聚合 statsd
指标并显示它们。
