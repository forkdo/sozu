...
sozu 可执行文件可用于启动代理和配置它：添加新的后端服务器、读取指标等。
它通过一个 unix 套接字与当前运行的代理通信。

您可以通过在 `config.toml` 中添加以下内容来指定其路径：

```toml
command_socket = "path/to/your/command_folder/sock"
```

## 添加一个带有 http 和 https 前端的集群

首先，您需要创建一个具有 id 和负载均衡策略（roundrobin 或 random）的新集群：
...
最后，您必须创建一个前端以允许 sozu 将流量从监听器发送到您的后端：
